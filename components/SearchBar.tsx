...
export default function SearchBar({ polls }: { polls: PollSearchItem[] }) {
  ...
  const categories = useMemo(() => {
    ...
    return unique.filter((category) => category.toLowerCase().includes(normalizedQuery))
  }, [normalizedQuery, polls])
                  ...
                  filteredPolls.length > 0 ? (
                    filteredPolls.map((poll) => (
                      ...
                      >
                        ...
                        <p className="text-xs text-gray-500">{poll.category ?? 'Kategori saknas'}</p>
                      </Link>
                    ))
                  ) : (
                    <p className="text-sm text-gray-500">Inga omröstningar matchar sökningen</p>
                  )
                ) : (
